module Ecds
  class CombinedManifestController < ApplicationController
    def show
      ...
      photograph_manifest = record.manifests.find do |manifest|
        manifest.identifier.ends_with?(photograph_relationship_uuid)
      end

      # If no photograph manifest is found, return basic IIIF manifest
      manifest = if photograph_manifest.nil?
                   generate_empty_manifest(record)
                 else
                   JSON.parse(photograph_manifest.content, symbolize_names: true)
                 end
      ...
      related_records.each do |related_record|
        related_photograph_manifest = related_record.manifests.find do |rel_manifest|
          rel_manifest.identifier.ends_with?(photograph_relationship_uuid)
        end
